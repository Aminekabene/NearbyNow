...
function usePreventScroll(options) {
  ...
  utils.useLayoutEffect(function () {
    if (isDisabled) {
      return;
    }
...
function preventScrollStandard() {
  return utils.chain(setStyle(document.documentElement, 'paddingRight', window.innerWidth - document.documentElement.clientWidth + "px"), setStyle(document.documentElement, 'overflow', 'hidden'));
...
function preventScrollMobileSafari() {
  ...
  var restoreStyles = utils.chain(setStyle(document.documentElement, 'paddingRight', window.innerWidth - document.documentElement.clientWidth + "px"), setStyle(document.documentElement, 'overflow', 'hidden'), setStyle(document.body, 'marginTop', "-" + scrollY + "px")); // Scroll to the top. The negative margin on the body will make this appear the same.

  window.scrollTo(0, 0);
  var removeEvents = utils.chain(addEvent(document, 'touchstart', onTouchStart, {
    passive: false,
    capture: true
...
function scrollIntoView(target) {
  ...
  while (target && target !== root) {
    ...
    var scrollable = utils.getScrollParent(target);
...
var Sheet = /*#__PURE__*/React.forwardRef(function (_ref, ref) {
  ...
  var y = framerMotion.useMotionValue(0); // Keep the callback fns up-to-date so that they can be accessed inside
  ...
  var onDragEnd = useEvent(function (_, _ref3) {
    ...
    if (velocity.y > DRAG_VELOCITY_THRESHOLD) {
      ...
      framerMotion.animate(y, snapTo, animationOptions);
  ...
  var dragProps = React.useMemo(function () {
    var dragProps = {
      drag: 'y',
      dragElastic: 0,
      dragConstraints: {
  ...
  }), React.createElement(framerMotion.AnimatePresence, null, isOpen ? React.Children.map(children, function (child, i) {
    return React.cloneElement(child, {
      key: "sheet-child-" + i
    });
  }) : null)));
  if (IS_SSR) return sheet;
  return ReactDOM.createPortal(sheet, mountPoint != null ? mountPoint : document.body);
});
...
var SheetHeader = /*#__PURE__*/React.forwardRef(function (_ref, ref) {
  ...
  }, _dragProps), children || React.createElement("div", {
